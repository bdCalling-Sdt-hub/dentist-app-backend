...
const deleteArticleCategoryToDB = async (
  id: string,
): Promise<IArticleCategory | null> => {
  const isExist = await ArticleCategory.findById(id);
  if (!isExist) {
    throw new ApiError(
      StatusCodes.BAD_REQUEST,
      "Article category doesn't exist!",
  ...
  const deleteArticleCategory = await ArticleCategory.findByIdAndDelete(id);
